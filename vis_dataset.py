...
def draw_skeleton(img, kpt, connection=None, colors=None, bbox=None):
    ...
    elif npart==24: # smpl(24 joints + transl)
...
def vis_kpt_2d(json_file, ImgDir, output_dir):
    ...
    for img_id in tqdm(image_ids):
        ...
        img = cv2.imread(os.path.join(ImgDir, data['image_path']))
        ...
        # cv2.imshow('img', img)
        # cv2.waitKey()
        img_file = os.path.join(output_dir, data['image_path'])
...
if __name__ == "__main__":
    import argparse
    # sys.argv = ["", "--json_file", "results/VCLMP-train/VCLMP-train.json", "--ImgDir", "results/VCLMP-train/images", "--output_dir", "output"]
